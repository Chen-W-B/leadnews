...
public class WmNewsServiceImpl  extends ServiceImpl<WmNewsMapper, WmNews> implements WmNewsService {
    ...
    public ResponseResult findAll(WmNewsPageReqDto dto) {
        ...
        responseResult.setData(page.getRecords());//getRecords()获取分页查询结果的记录列表
    ...
    public ResponseResult submitNews(WmNewsDto dto) {
        ...
        if (dto.getImages() != null && dto.getImages().size() > 0){
            ...
            wmNews.setImages(imageStr); //imageStr="1dddfsd.jpg,sdlfjldk.jpg"
        }
        //如果当前封面类型为自动 -1
    ...
    private List<String> extractUrlInfo(String content) {
        //content包含两个属性：type(值为text或者image)、value
        //  type=text时，value=文本内容
        //  type=image时，value=图片地址

        List<String> materials = new ArrayList<>();
        //parseArray(字符串,解析后的对象类型)：用于将JSON字符串解析成Java的List对象。这个方法在处理包含数组结构的JSON数据时非常有用。
