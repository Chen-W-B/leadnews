...
public class ApArticleServiceImpl extends ServiceImpl<ApArticleMapper, ApArticle> implements ApArticleService {
    ...
    private final static Integer MAX_PAGE_SIZE = 50;// 单页最大加载的数字
    ...
    public ResponseResult load(Short loadtype, ArticleHomeDto dto) {
        ...
        if (! loadtype.equals(ArticleConstants.LOADTYPE_LOAD_MORE) && ! loadtype.equals(ArticleConstants.LOADTYPE_LOAD_NEW)){
            //既不是加载更多，也不是加载最新。那就默认将它赋值为 加载更多
            loadtype = ArticleConstants.LOADTYPE_LOAD_MORE;
        }
        //1.3文章频道校验
        if (StringUtils.isBlank(dto.getTag())){
            //没有指定哪个文章频道，那就将他赋值为默认频道
    ...
    public ResponseResult saveArticle(ArticleDto dto) {
        ...
        //测试feign的开启服务降级，也可以指定服务响应的超时的时间(2s=2000ms)
        //try {
        //    Thread.sleep(3000);//睡眠3秒，已超出2s
        //    //会报错 --  java.lang.RuntimeException: WmNewsAutoScanServiceImpl-文章审核，保存app端相关文章数据失败
